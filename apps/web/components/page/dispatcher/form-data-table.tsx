...
interface FormDataTableProps {
  data: FormDataEntry[];
  onChange: (data: FormDataEntry[]) => void;
}

export function FormDataTable({ data, onChange }: FormDataTableProps) {
  const addRow = () => {
    onChange([
      ...data,
      {
  ...
  return (
    <div className="space-y-3">
          ...
          <tbody>
            {data.length === 0 ? (
              <tr>
                ...
                >
                  <p className="text-sm">
                    No form data entries. Click "Add Parameter" to start.
                  </p>
                </td>
              </tr>
            ) : (
      ...
      >
        <Plus className="size-4 mr-2" /> Add Parameter
      </Button>
    </div>
  );
}
